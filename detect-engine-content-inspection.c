                                  ...
                                  uint8_t inspection_mode)
{
    ...
    //struct Position *p_one_tmp=NULL;
    ...
    if (smd->type == DETECT_CONTENT) {
        ...
        do {
            ...
            if (found == NULL && !(cd->flags & DETECT_CONTENT_NEGATED)) {
                ...
                SCLogInfo("content %"PRIu32" matched at offset %"PRIu32"", cd->id, match_offset);
        ...
        do {
            ...
            if (!(pe->flags & DETECT_PCRE_RELATIVE_NEXT)) {
                ...
                /*位置传入结构体*/
                if(f){
                    Get_gaoliang((char*)buffer,det_ctx->buffer_offset,det_ctx->buffer_offset-det_ctx->pcre_match_start_offset+1,f->gaoliang);
                }
                SCLogInfo("pcre matched at end %"PRIu32" depth %"PRIu32" ",det_ctx->buffer_offset,det_ctx->buffer_offset-det_ctx->pcre_match_start_offset);
